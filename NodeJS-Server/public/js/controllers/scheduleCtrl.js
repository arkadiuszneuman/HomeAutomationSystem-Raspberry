homeAutomationApp.controller('scheduleCtrl', function($scope, $http, $state, $stateParams) {
    $scope.schedules = [];
    var device = null;
    $http.get('api/device/' + $stateParams.id).success(function(data) {
        device = data;
    ...
    $scope.save = function(schedule) {
        ...
        if (schedule._id == null) {
            $http.put('api/device/' + device._id + '/schedule', schedule).success(function(data) {
            ...
            $http.post('api/device/' + device._id + '/schedule', schedule).success(function(data) {
                schedule.saving = false;
                schedule._id = data._id;
            }).error(function(err) {
                console.log(err);
    ...
    $scope.delete = function(schedule) {
        ...
        if (schedule._id != null) {
            $http.delete('api/device/' + device._id + '/schedule/' + schedule._id).success(function() {
    ...
    $scope.changeType = function(type) {
        $state.go('home.receivers.schedule.' + type);
    }

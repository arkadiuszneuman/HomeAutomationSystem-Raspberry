...
router.post('/device/:id/schedule', function (req, res) {
    ...
    models.Device.update({ _id: req.params.id, 'schedule._id': req.body._id }, {
        '$set': {
            'schedule.$.name': scheduleToAdd.name,
            'schedule.$.desc': scheduleToAdd.desc,
            'schedule.$.cron': scheduleToAdd.cron,
            'schedule.$.mainType': scheduleToAdd.mainType,
            'schedule.$.subType': scheduleToAdd.subType,
            'schedule.$.difference': scheduleToAdd.difference,
            'schedule.$.status': scheduleToAdd.status
        }
    }, function (err) {
        if (err)
            logger.info(err);
